...
class conv_net():
    ...
    def nn_layer(input_dim, output_dim, layer_name, initializer="xavier"):
    ...
    def __set_variable__(self, initializer, strides=[1,2,1,2]):
        ...
        full_size = int(self.input_size/(strides[1]**2)/(strides[3]**2))
        nn_layer([5,5,1], [32], "conv1", initializer)
        nn_layer([5,5,32], [64], "conv2", initializer)
        nn_layer([full_size*64], [1024], "fcon1", initializer)
        nn_layer([1024], [self.n_output], "output", initializer)
    ...
    def network(self, x, n_input, n_output, dropout, strides, initializer="xavier"):
        ...
        x = tf.reshape(x, shape=[-1, np.sqrt(n_input), np.sqrt(n_input), 1])
        with tf.variable_scope('conv1'):
            # Convolution Layer
            conv1 = conv2d(x, self.variables["conv1_w"], self.variables["conv1_b"], strides = strides[0])
            tf.histogram_summary(self.name+"/conv1/conv_output", conv1)
            # Max Pooling (down-sampling)
            conv1 = maxpool2d(conv1, k=strides[1])
            tf.histogram_summary(self.name+"/conv1/maxpool_output", conv1)
        with tf.variable_scope('conv2'):
            # Convolution Layer
            conv2 = conv2d(conv1, self.variables["conv2_w"], self.variables["conv2_b"], strides = strides[2])
            tf.histogram_summary(self.name+"/conv2/conv_output", conv2)
            # Max Pooling (down-sampling)
            conv2 = maxpool2d(conv2, k=strides[3])
            tf.histogram_summary(self.name+"/conv2/maxpool_output", conv2)
        with tf.variable_scope('fcon1'):
            ...
            tf.histogram_summary(self.name+"/fcon1/fcon1_output", fc1)
            fc1 = tf.nn.relu(fc1)
            # Apply Dropout
            fc1 = tf.nn.dropout(fc1, dropout)
            tf.histogram_summary(self.name+"/fcon1/dropout_output", fc1)
        with tf.variable_scope('output'):
            # Output
            self.out = tf.add(tf.matmul(fc1, self.variables["output_w"]), self.variables["output_b"])
            tf.histogram_summary(self.name+"/output/preact_output", self.out)
